...
Approach: If its ugly, like me, then we can just change the order with the max with min so that the sum now wont equal with the next elem
*/
...
int main() {
    ...
    while (t--) {
        ...
        if(isBeautiful(a)) {
            ...
            int right = n-1;
            while(left<=right){
                if(left < n and right > -1) swap(a[left], a[right]);
                left += 2;
                right -= 2;
            }
            if(isBeautiful(a)) {
